...
export function WalletApprovalActions({ circleId, requestId }: { circleId: string; requestId: string }) {
  const router = useRouter()
  const [loading, setLoading] = useState<"approve" | "reject" | null>(null)

  async function act(action: "approve" | "reject") {
    setLoading(action)
    const res = await fetch(`/api/circles/${circleId}/wallet/approvals/${requestId}/${action}`, { method: "POST" })

...
export async function POST(req: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    if (!VALID_ROLES.includes(finalRole)) return NextResponse.json({ error: "Invalid role" }, { status: 400 })
    ...
    const member = await prisma.circleMember.create({ data: { circleId, userId, role: finalRole } })

...
import { getProjectDistributionDashboard, createProfitDistribution } from "@/lib/services/project-distribution.service"
...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  try {
    ...
    return NextResponse.json(await createProfitDistribution(projectId, circleId, s.user.id, body), { status: 201 })
  } catch (e) {

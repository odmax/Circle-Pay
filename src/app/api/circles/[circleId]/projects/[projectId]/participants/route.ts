...
import { getProjectParticipants, addExternalParticipant, acceptParticipant, removeParticipant } from "@/lib/services/project-funding.service"
...
export async function GET(_req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  ...
  return NextResponse.json(await getProjectParticipants(projectId))
}
...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  ...
  try {
    ...
    if (action === "add") return NextResponse.json(await addExternalParticipant(projectId, body), { status: 201 })
    if (action === "accept") { await acceptParticipant(body.participantId); return NextResponse.json({ ok: true }) }
    if (action === "remove") { await removeParticipant(body.participantId); return NextResponse.json({ ok: true }) }

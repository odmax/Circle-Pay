...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  ...
  try {
    const allowed = await hasCirclePermission({ userId: s.user.id, circleId, permission: CIRCLE_PERMISSIONS.FUNDING_ALLOCATION_MANAGE })
    if (!allowed) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
    const body = await req.json()
    if (action === "generate") return NextResponse.json(await generateEqualAllocations(body.fundingRoundId, s.user.id), { status: 201 })
    if (action === "adjust") return NextResponse.json(await adjustAllocation(body.allocationId, s.user.id, { allocatedAmount: body.allocatedAmount, reason: body.reason }))

...
import { rejectExpense } from "@/lib/services/project-expense.service"
...
export async function POST(
  ...
  try {
    ...
    const canApprove = await hasCirclePermission({ userId: s.user.id, circleId, permission: CIRCLE_PERMISSIONS.PROJECT_EXPENSE_APPROVE })
    if (!canApprove) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
    const { reason } = await req.json().catch(() => ({}))
    return NextResponse.json(await rejectExpense(expenseId, s.user.id, reason))
  } catch (e) {

...
import { voidExpense } from "@/lib/services/project-expense.service"
...
export async function POST(
  ...
  try {
    ...
    const canVoid = await hasCirclePermission({ userId: s.user.id, circleId, permission: CIRCLE_PERMISSIONS.PROJECT_EXPENSE_VOID })
    if (!canVoid) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
    const { reason } = await req.json()
    return NextResponse.json(await voidExpense(expenseId, s.user.id, reason))
  } catch (e) {

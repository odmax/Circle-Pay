...
import { listProjectUpdates, createProjectUpdate } from "@/lib/services/investor-relations.service"
...
export async function GET(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  ...
  return NextResponse.json({ updates: await listProjectUpdates(projectId, circleId, ctx) })
}
...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  try {
    ...
    const update = await createProjectUpdate(projectId, circleId, ctx.userId, body)

...
import { listProjectMeetings, scheduleInvestorMeeting } from "@/lib/services/investor-relations.service"
...
export async function GET(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  ...
  return NextResponse.json({ meetings: await listProjectMeetings(projectId, circleId) })
}
...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string; projectId: string }> }) {
  try {
    ...
    const m = await scheduleInvestorMeeting(circleId, projectId, ctx.userId, body)

...
import { createHouseholdRoom, updateHouseholdRoom, deleteHouseholdRoom } from "@/lib/services/household-lease.service"
...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string }> }) {
  try {
    ...
    return NextResponse.json(await createHouseholdRoom(circleId, ctx.userId, await req.json()), { status: 201 })
  } catch (e) {

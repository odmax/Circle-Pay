...
import { generateMemberStatementData } from "@/lib/services/statement.service"
...
export async function GET(
  ...
  try {
    ...
    if (format === "pdf") {
      ...
      const fileName = `statement-${data.member.name.replace(/\s+/g, "-")}-${data.period.from}-to-${data.period.to}.pdf`
    ...
    const msg = error instanceof Error ? error.message : "Failed to generate member statement"

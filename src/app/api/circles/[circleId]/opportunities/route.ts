...
export async function GET(req: Request, { params }: { params: Promise<{ circleId: string }> }) {
  ...
  return NextResponse.json(await listOpportunities(circleId, s.user.id))
}
...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string }> }) {
  try {
    ...
    if (!s?.user?.id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
    const { circleId } = await params
    const allowed = await hasCirclePermission({ userId: s.user.id, circleId, permission: CIRCLE_PERMISSIONS.OPPORTUNITY_CREATE })
    if (!allowed) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
    const body = await req.json()
    const opp = await createOpportunity(circleId, s.user.id, body)

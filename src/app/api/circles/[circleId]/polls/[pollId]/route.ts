...
export async function POST(req: Request, { params }: { params: Promise<{ circleId: string; pollId: string }> }) {
  ...
  try {
    ...
    const poll = await prisma.circlePoll.findUnique({ where: { id: pollId, circleId }, select: { id: true } })
    if (!poll) return NextResponse.json({ error: "Poll not found in this circle" }, { status: 404 })
